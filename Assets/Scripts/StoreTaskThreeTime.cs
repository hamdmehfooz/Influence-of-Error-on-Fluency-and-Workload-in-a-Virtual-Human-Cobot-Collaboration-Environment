...
public class StoreTaskThreeTime : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (StartIndicator.activeInHierarchy && !isTiming && !hasLogged)
        {
            StartTimer();
        }

        // Stop timer and log when object B becomes active (only if not logged before)
        if (EndIndicator.activeInHierarchy && isTiming && !hasLogged)
        {
            StopTimer();
        }
    }
    ...
    void StopTimer()
    {
        ...
        Debug.Log($"Timer Stopped: end indicator is active. Time elapsed: {minutes}m {seconds}s {milliseconds}ms");

        // Write elapsed time to CSV file
        File.AppendAllText(filePath, $"Task 3,{DateTime.Now:yyyy-MM-dd HH:mm:ss},{minutes},{seconds},{milliseconds}\n");
    }
}
